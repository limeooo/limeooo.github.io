  ...
  Picker.prototype = {
    ...
    _renderCol: function () {
      ...
      for (var i = 0, len = this.data.length; i < len; i++) {
        ...
        for (var j = 0, len2 = this.data[i].length; j < len2; j++) {
          item += `<div id="${this.data[i][j].value}" class="picker-item">${this.data[i][j].title}</div>`
        }
        group +=
          '<div class="picker-items-col"><div class="picker-items-col-wrapper">' +
          item +
          '</div></div>'
      }
      return group
    ...
    _renderPop: function () {
      ...
      if (cols && !qs('#cover') && !qs('#picker')) {
        ...
        this.picker.innerHTML =
          '<div class="picker-header"><span class="cancel">取消</span><span class="title">实习单位范围</span><span class="confirm">确定</span></div><div class="picker-items">' +
          cols +
          '<div class="picker-line picker-line-top"></div><div class="picker-line picker-line-bottom"></div></div>'
        ...
        for (var i = 0; i < this.data.length; i++) {
          ...
          if (this.default && typeof this.default === 'object' && this.default[i]) {
            ...
            const activeEl = document.getElementById(this.data[i][_index].value)
            ...
            const activeEl = document.getElementById(this.data[i][0].value)
            activeEl.classList.add('active')
          }
    ...
    _setOffset: function (el, index, n, isTouchEnd) {
      ...
      if (isiOS) {
        el.querySelector('.picker-items-col-wrapper').style.cssText =
          ...
          ((this._height * 6) / 2 - this._height / 2 - n) +
      ...
      for (var i = 0, len = this.data[index].length; i < len; i++) {
        if (-i * this._height + n > -9 && -i * this._height + n <= 9) {
          ...
          if (isTouchEnd) {
            ...
            const oldEl = document.getElementById(this._selected[index].value)
            const newEl = document.getElementById(res.value)
    ...
    _listener: function (el, index, offsetNum) {
      ...
      function touchendtHandler(e) {
        ...
        var _this = this
        touchMovedY =
          touchChangedY % self._height > self._height / 2
            ? Math.ceil(touchChangedY / self._height) * self._height
            : Math.floor(touchChangedY / self._height) * self._height
        if (touchMovedY < 0) {
          touchMovedY = 0
        ...
        function inertiaMove() {
          ...
          if (Math.abs(dist) < 0.5) {
            touchMovedY =
              touchMovedY % self._height > self._height / 2
                ? Math.ceil(touchMovedY / self._height) * self._height
                : Math.floor(touchMovedY / self._height) * self._height
            self._setOffset(_this, thisIndex, touchMovedY, true)
            return
          }
    ...
    _renderItem: function (config) {
      ...
      for (var i = 0, len = config.data.length; i < len; i++) {
        _item += `<div id="${config.data[i].value}" class="picker-item">${config.data[i].title}</div>`
      }
      ...
      this._setOffset(
        el,
        config.index,
        getIndex(config.default, config.data) * this._height || 0,
        true
      )
      this._listener(
        el,
